...
import (
	...
	"backend/user-service/internal/config"
...
func main() {
	...
	logger := logging.New("user-service", cfg.AppEnv)
	...
	healthHandler := health.ReadyHandler("1.0.0", readiness, map[string]health.Checker{
		"database": userService.Ping,
		"rabbitmq": rabbitCheck,
	})
	root := handler.New(userService, cfg.InternalSecret, healthHandler).Routes()
	...
	readiness.MarkReady()
	go func() {
		logger.Info().Str("port", cfg.AppPort).Msg("user_service_started")
