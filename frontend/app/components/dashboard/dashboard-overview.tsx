...
import { getFakePromos } from "@/lib/demo-data/catalog";
import type {
...
export function DashboardOverview() {
  ...
  const [details, setDetails] = useState<DetailedAnalyticsResponse | null>(
    null,
  );
  ...
  useEffect(() => {
    async function loadDashboardAnalytics() {
      try {
        const [mergeResponse, detailsResponse, promoResponse] =
          await Promise.all([
            getMergeStats(),
            getShopDetails(),
    ...
    <main className="dashboard-main">
      <section className="dashboard-hero">
        <div className="dashboard-hero-panel">
          ...
          <h2>
            Pilotez les chiffres retail et parapharmacie depuis un seul cockpit.
          </h2>
          <p>
            Vue instantanee sur {formatDashboardNumber(summary.totalProducts)}{" "}
            references, {formatDashboardNumber(summary.totalShops)} boutiques
            suivies et un prix moyen observe de{" "}
            {formatDashboardCurrency(summary.weightedAveragePrice)}.
          </p>
          <div className="dashboard-hero-strip">
            <span className="dashboard-hero-chip">
              {formatDashboardNumber(summary.retailProducts)} produits
              e-commerce
            </span>
            <span className="dashboard-hero-chip">
              {formatDashboardNumber(summary.paraProducts)} produits
              parapharmacie
            </span>
            <span className="dashboard-hero-chip">
              {formatDashboardNumber(summary.totalDiscounts)} remises suivies
            </span>
          </div>
        </div>
