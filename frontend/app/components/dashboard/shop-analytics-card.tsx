...
export function ShopAnalyticsCard({
  ...
  const availabilityPercent =
    productCount > 0 ? ((availableCount / productCount) * 100).toFixed(1) : 0;
  const discountPercent =
    productCount > 0 ? ((discountCount / productCount) * 100).toFixed(1) : 0;
      ...
      <CardContent className="space-y-4">
        ...
        <div>
          ...
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="font-medium text-stock">
              {totalDiscountValue.toFixed(2)} DT
            </span>
          </div>
          <div className="flex items-center justify-between text-xs">
            <span className="text-muted-foreground">Remise moyenne</span>
            <span className="font-medium text-purple">
              {averageDiscountPercent.toFixed(2)}%
            </span>
          </div>
        </div>
      </CardContent>
