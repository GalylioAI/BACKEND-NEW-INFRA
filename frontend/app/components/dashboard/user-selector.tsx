...
import { getApiErrorMessage, listAdminUsers } from "@/lib/api";
import { cn } from "@/lib/utils";

type SelectorRole = "user" | "admin" | "superadmin" | "both";
...
export function UserSelector({
  ...
  const allowedRoles = useMemo(() => {
    if (filterRole === "both") return ["user", "admin", "superadmin"] as const;
  ...
  return (
    <div className="space-y-2">
      <label className="block text-xs font-medium text-muted-foreground">
        SPECIFIC USERS{" "}
        {filterRole !== "both" && `(${filterRole.toUpperCase()})`}
      </label>
