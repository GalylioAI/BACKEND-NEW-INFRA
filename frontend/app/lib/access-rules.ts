import type { AccessRule, DashboardRole, UserResponse } from "@/lib/api/types";
...
function scoreAccessRulePath(pattern: string) {
  ...
  const exactSegments = segments.filter((segment) => !segment.includes("[") && segment !== "*").length;
  const dynamicSegments = segments.filter((segment) => segment.includes("[")).length;
...
export function findActiveAccessRule(rules: AccessRule[], pathname: string) {
  const dedupedRules = Array.from(
    rules.reduce((map, rule) => {
      ...
      const currentUpdatedAt = current?.updated_at ? Date.parse(current.updated_at) || 0 : 0;
      const nextUpdatedAt = rule.updated_at ? Date.parse(rule.updated_at) || 0 : 0;
      ...
      return map;
    }, new Map<string, AccessRule>()).values(),
  );

  return dedupedRules
    .filter((rule) => matchesAccessRulePath(rule.path, pathname))
    .sort((left, right) => scoreAccessRulePath(right.path) - scoreAccessRulePath(left.path))[0] || null;
}

export function canUserAccessRule(rule: AccessRule | null, user: UserResponse | null) {
  if (!rule) return true;

  const allowedRoles = rule.allowed_roles || [];
  const allowedEmails = (rule.allowed_emails || []).map((email) => email.toLowerCase());
  ...
  const userRole = (user.role || "client") as DashboardRole;
