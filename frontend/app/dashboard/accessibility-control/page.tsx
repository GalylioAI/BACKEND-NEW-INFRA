...
import type { AccessRule } from "@/lib/demo-data/types";
...
type PageDefinition = {
  ...
  visible: boolean;
  role: "user" | "admin" | "superadmin" | "both";
};
...
function getInitialRole(rule?: AccessRule) {
  ...
  if (roles.includes("user") && roles.includes("admin") && roles.length === 2)
    return "both";
  return "both";
}
...
export default function AccessibilityControlPage() {
  ...
  const [customCategory, setCustomCategory] =
    useState<AccessRule["category"]>("Custom");
  ...
  const ruleByPath = useMemo(
    () => new Map(rules.map((rule) => [rule.path, rule])),
    [rules],
  );
  ...
  const customRules = useMemo(() => {
    return rules.filter(
      (rule) => !pages.some((page) => page.path === rule.path),
    );
  }, [rules]);
  ...
  const visibleCount =
    mergedPages.filter((page) => page.visible).length +
    customRules.filter((rule) => rule.visible).length;
  const restrictedCount = pages.filter((page) => page.role !== "both").length;
  const dashboardCount = pages.filter(
    (page) => page.category === "Dashboard",
  ).length;
  ...
  const sections = categoryOrder
    ...
    .filter(
      (section) =>
        selectedCategory === "all" || selectedCategory === section.category,
    );
  ...
  const handleCreateCustomRule = async () => {
    ...
    try {
      ...
      const savedRule = await upsertAccessRule({
        ...
        allowed_roles: customRole === "both" ? ["user", "admin"] : [customRole],
        allowed_emails: customAllowedUsers,
      });

      setRules((currentRules) => {
        const withoutPrevious = currentRules.filter(
          (rule) => rule.path !== savedRule.path,
        );
      ...
      setLoadError(
        error instanceof Error ? error.message : "Unable to create access rule",
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleDeleteCustomRule = async (path: string) => {
    ...
    try {
      await deleteAccessRule(path);
      setRules((currentRules) =>
        currentRules.filter((rule) => rule.path !== path),
      );
    } catch (error) {
      setLoadError(
        error instanceof Error ? error.message : "Unable to delete access rule",
      );
    } finally {
      setSaving(false);
    }
              ...
              <div className="max-w-3xl space-y-3">
                ...
                <div>
                  ...
                  <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                    Control visibility, roles, and user-specific access from one
                    clear overview. Save each route to persist it in
                    browser-local demo state, or add a brand new URL below.
                  </p>
                </div>
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-3 xl:w-[430px]">
                <div className="rounded-2xl border border-border bg-background p-4 shadow-sm">
                  ...
                  <strong className="mt-3 block text-2xl font-black text-foreground">
                    {pages.length + customRules.length}
                  </strong>
                  <p className="mt-1 text-xs leading-5 text-muted-foreground">
                    Total routes under access control.
                  </p>
                </div>
                <div className="rounded-2xl border border-border bg-background p-4 shadow-sm">
                  <div className="flex items-center justify-between gap-3">
                  ...
                  <p className="mt-1 text-xs leading-5 text-muted-foreground">
                    Pages currently available to users.
                  </p>
                </div>
                <div className="rounded-2xl border border-border bg-background p-4 shadow-sm">
                  <div className="flex items-center justify-between gap-3">
                  ...
                  <p className="mt-1 text-xs leading-5 text-muted-foreground">
                    Routes using role-based access.
                  </p>
                </div>
              </div>
            </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm leading-6 text-blue-500/80">
                      Edit a page, choose a role, attach user emails, then save.
                      If you need a brand new URL, use the form below and it
                      will be added to the same local demo map.
                    </p>
                  </div>
                </div>
              </div>
        ...
        <section className="dashboard-card">
          <div className="dashboard-card-header">
            <div>
              ...
              <p className="dashboard-card-subtitle">
                Create a new route rule without leaving the page.
              </p>
            </div>
          </div>
          ...
          <div className="dashboard-card-body space-y-4">
            <div className="grid gap-4 lg:grid-cols-2">
              <div className="space-y-3">
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  <select
                    ...
                    onChange={(event) =>
                      setCustomCategory(
                        event.target.value as AccessRule["category"],
                      )
                    }
                  ...
                  <select
                    value={customRole}
                    onChange={(event) =>
                      setCustomRole(event.target.value as typeof customRole)
                    }
                    className="h-11 rounded-2xl border border-border bg-background px-4 text-sm text-foreground shadow-sm focus:outline-none focus:ring-2 focus:ring-purple/20"
                  >
                  ...
                  <label className="flex items-center gap-2 rounded-2xl border border-border bg-background px-4 text-sm text-muted-foreground shadow-sm">
                    <input
                      ...
                      onChange={(event) =>
                        setCustomVisible(event.target.checked)
                      }
                    />
                    Visible
                  </label>
              ...
              <div className="rounded-2xl border border-border bg-background p-4 shadow-sm">
                <UserSelector
                  selectedUsers={customAllowedUsers}
                  onUsersChange={setCustomAllowedUsers}
                />
              </div>
            </div>
        ...
        <section className="dashboard-card">
          <div className="dashboard-card-header">
            <div>
              ...
              <p className="dashboard-card-subtitle">
                Search by page name or route, then narrow the control surface by
                section.
              </p>
            </div>
          </div>
          ...
          <div className="dashboard-card-body space-y-4">
            ...
            <div className="flex flex-wrap gap-2 text-xs font-semibold text-muted-foreground">
              ...
              <span className="rounded-full border border-border bg-background px-3 py-1">
                {dashboardCount} dashboard pages
              </span>
              <span className="rounded-full border border-border bg-background px-3 py-1">
                Live updates apply instantly after save
              </span>
            </div>
          </div>
        </section>
          ...
          <section className="dashboard-card">
            <div className="flex flex-col items-center justify-center px-6 py-14 text-center">
              ...
              <p className="mt-1 max-w-md text-sm leading-6 text-muted-foreground">
                Try another search term or switch category to reveal the pages
                you want to manage.
              </p>
            </div>
          </section>
                    ...
                    {section.pages.map((page) => (
                      <PageAccessCard
                        ...
                        initialSelectedUsers={
                          ruleByPath.get(page.path)?.allowed_emails || []
                        }
                      />
                    ))}
                  </div>
              ...
              <section className="dashboard-card">
                <div className="dashboard-card-header">
                  <div>
                    ...
                    <p className="dashboard-card-subtitle">
                      Rules that do not belong to the default page catalog.
                    </p>
                  </div>
                  <span className="dashboard-badge">
                    {filteredCustomRules.length} rules
                  </span>
                </div>
                <div className="dashboard-card-body">
