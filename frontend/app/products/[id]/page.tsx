...
import { getCatalogProduct, listCatalogProducts } from "../../lib/api/products";
import type { CatalogProduct, CatalogSource, CategoryType } from "../../lib/api/types";
import { getStoredProductImage } from "../../lib/product-image-store";
import { isLikelyPlaceholderProductImage } from "../../lib/product-utils";
import {
  bestShopName,
  formatPrice,
  normalizeShopName,
...
function specifications(product: CatalogProduct) {
  ...
  return [
    ...
    { label: "Disponibilité", value: product.inStock ? "En stock" : "Indisponible" },
...
function extractProductImages(product: CatalogProduct): string[] {
  ...
  const maybePush = (value: unknown) => {
    ...
    if (Array.isArray(value)) {
      value.forEach((entry) => {
        if (typeof entry === "string" && entry.trim()) specCandidates.push(entry);
      });
    }
  };
  ...
  return [...new Set([product.image, ...imagesFromArray, ...specCandidates].filter(Boolean))];
}

function categoryParams(product: CatalogProduct, source: CatalogSource): { category?: string; category_type?: CategoryType } {
  if (source === "para" && "topCategory" in product && product.topCategory) {
    return { category: product.topCategory, category_type: "top_category" };
  }
  if (product.category) return { category: product.category, category_type: "subcategory" };
  return {};
}

async function getRecommendations(product: CatalogProduct, source: CatalogSource) {
  try {
    const response = await listCatalogProducts(source, { ...categoryParams(product, source), limit: 6, page: 1 });
    return response.products.filter((item) => item.id !== product.id).slice(0, 5);
  } catch { return []; }
}

/* ── Chart helpers ──────────────────────────────────────────────────── */
const SHOP_COLORS = ["#56B4E9", "#E69F00", "#CC79A7", "#0072B2", "#D55E00", "#F0E442"];
...
function buildPriceSeries(shopName: string, startPrice: number, endPrice: number, points = 12): number[] {
...
function buildChartSeries(product: CatalogProduct): ShopSeries[] {
  ...
  return shops.slice(0, 4).map((shop, idx) => {
    ...
    const old = shop.oldPrice && shop.oldPrice > current ? shop.oldPrice : current * 1.06;
    return {
...
export default async function ProductDetailsPage({ params, searchParams }: ProductDetailsPageProps) {
  ...
  const preferredSource = query.source === "para" || query.source === "retail" ? query.source : undefined;
  ...
  function resolveImage(p: CatalogProduct): string {
    if (isLikelyPlaceholderProductImage(p.image)) {
      const stored = getStoredProductImage(p.id);
      if (stored && !isLikelyPlaceholderProductImage(stored)) return stored;
    }
    return p.image;
  }

  const productImage = resolveImage(product);
  const productImages = extractProductImages({ ...product, image: productImage });
  const category = productCategory(product);
  const chartSeries = buildChartSeries(product);
  const chartMonths = buildMonths(12);
  const availableShopsCount = stores.filter((shop) => (shop.available ?? true)).length;
