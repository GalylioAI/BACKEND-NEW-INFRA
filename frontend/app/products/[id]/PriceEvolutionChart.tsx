...
export default function PriceEvolutionChart({ series, months, bestPrice, originalPrice }: Props) {
  const isLight = useIsLight();
  const [activeShop, setActiveShop] = useState<string | null>(null);
  const [hover, setHover] = useState<{ si: number; pi: number } | null>(null);
  const svgScopeId = useId().replace(/:/g, "-");

  const gridStroke = isLight ? "rgba(91,33,182,0.08)" : "rgba(255,255,255,0.06)";
  ...
  const legendColor = (active: boolean) =>
    isLight
      ? active ? "rgba(30,27,75,0.9)" : "rgba(30,27,75,0.3)"
      : active ? "rgba(240,240,245,0.85)" : "rgba(240,240,245,0.3)";
  const tooltipBg = isLight ? "#ffffff" : "#16161f";
  const tooltipTextMuted = isLight ? "rgba(30,27,75,0.45)" : "rgba(240,240,245,0.45)";
  const tooltipTextSoft = isLight ? "rgba(30,27,75,0.5)" : "rgba(240,240,245,0.5)";
  const statPillBg = isLight ? "rgba(91,33,182,0.08)" : "rgba(59,222,185,0.1)";
  const statPillBorder = isLight ? "rgba(91,33,182,0.18)" : "rgba(59,222,185,0.2)";
  const statPillColor = isLight ? "#5B21B6" : "#3BDEB9";
  const oldPriceBg = isLight ? "rgba(30,27,75,0.05)" : "rgba(255,255,255,0.04)";
  const oldPriceBorder = isLight ? "rgba(30,27,75,0.08)" : "rgba(255,255,255,0.08)";
  const oldPriceColor = isLight ? "rgba(30,27,75,0.5)" : "rgba(240,240,245,0.5)";
  ...
  const tooltip = useMemo(() => {
    ...
    return { x, y, shop: s.shop, color: s.color, value: v, month: months[hover.pi] };
    ...
    <div style={{ position: "relative", width: "100%", fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10, flexWrap: "wrap" }}>
        {series.map((s) => (
          <button
            ...
            style={{
              ...
              borderRadius: 8,
              outline: activeShop === s.shop ? `1px solid ${s.color}` : "1px solid transparent",
              transition: "outline 0.15s ease",
            }}
          >
            <span style={{ width: 24, height: 3, borderRadius: 99, background: s.color, display: "block", flexShrink: 0 }} />
            <span style={{ fontSize: 12, fontWeight: 600, color: legendColor(activeShop === null || activeShop === s.shop), transition: "color 0.15s" }}>
              {s.shop}
            </span>
          </button>
        ))}
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 10, flexWrap: "wrap" }}>
          <div style={{ fontSize: 11, fontWeight: 700, background: statPillBg, border: `1px solid ${statPillBorder}`, borderRadius: 6, padding: "3px 10px", color: statPillColor }}>
            Actuel : {fmt(bestPrice)}
          </div>
          {originalPrice && originalPrice > bestPrice && (
            <div style={{ fontSize: 11, fontWeight: 700, background: oldPriceBg, border: `1px solid ${oldPriceBorder}`, borderRadius: 6, padding: "3px 10px", color: oldPriceColor, textDecoration: "line-through" }}>
              {fmt(originalPrice)}
            </div>
          )}
        </div>
      ...
      <div style={{ position: "relative", width: "100%", overflow: "visible" }}>
        ...
        >
          <defs>
            {series.map((s) => (
              <linearGradient key={s.shop} id={`area-${svgScopeId}-${toSafeId(s.shop)}`} x1="0" y1="0" x2="0" y2="1">
          ...
          {yTicks.map((tick) => {
            ...
            return (
              <g key={tick}>
                ...
                <text x={PAD.l - 8} y={yy + 4} textAnchor="end" fontSize={11} fill={labelFill} fontFamily="'Plus Jakarta Sans', sans-serif">
          ...
          {/* X axis baseline */}
          <line x1={PAD.l} y1={PAD.t + CH} x2={W - PAD.r} y2={PAD.t + CH} stroke={axisStroke} strokeWidth={1} />
          ...
          {months.map((m, i) => {
            ...
            if (!show) return null;
            return (
              <text key={i} x={px(i, months.length)} y={H - 6} textAnchor="middle" fontSize={11} fill={labelFill} fontFamily="'Plus Jakarta Sans', sans-serif">
                {m}
              </text>
            );
          })}
          ...
          <g clipPath={`url(#chart-clip-${svgScopeId})`}>
            {series.map((s) => {
              ...
              const pts = s.values.map((v, i) => `${px(i, months.length).toFixed(1)},${py(v, min, max).toFixed(1)}`);
              ...
              return (
                <g key={s.shop} style={{ transition: "opacity 0.2s" }} opacity={dimmed ? 0.15 : 1}>
                  <path d={areaPath} fill={`url(#area-${svgScopeId}-${toSafeId(s.shop)})`} />
          ...
          {series.map((s, si) => {
            ...
            return s.values.map((v, pi) => {
              ...
              return (
                <g key={`${si}-${pi}`}
                  onMouseEnter={() => { if (!dimmed) setHover({ si, pi }); }}
        ...
        {tooltip && (
          <div style={{
            ...
            <div style={{ fontSize: 10, color: tooltipTextMuted, marginBottom: 2 }}>{tooltip.month}</div>
            <div style={{ fontSize: 13, fontWeight: 800, color: tooltip.color }}>{fmt(tooltip.value)}</div>
            <div style={{ fontSize: 11, color: tooltipTextSoft, marginTop: 2 }}>{tooltip.shop}</div>
          </div>
        )}
      </div>

      {/* Footer note */}
      <p style={{ margin: "8px 0 0", fontSize: 10, color: footerColor, textAlign: "right", fontStyle: "italic" }}>
